...
<div class="search_view" id="workorder_abdicate_request_view">
<%
    if(((User)session.getAttribute("user")).getUserType().equals("HR")){
 %>
<form class="search_form" action="<%=request.getContextPath()%>/workflow/abdicate_request/query.action">
<%
    }else{
%>
<form class="search_form" action="<%=request.getContextPath()%>/workflow/abdicate_request/query.action?managerId=<%=((User)session.getAttribute("user")).getId()%>">
<%
    }
%>
...
(function($){
    ...
    initSearchView(id="workorder_abdicate_request_view",table_colums,"id",dategrid_toolbar);

...
<div class="search_view" id="workorder_apply_leave_request_view">
<%
    if(((User)session.getAttribute("user")).getUserType().equals("HR")){
%>
<form class="search_form" action="<%=request.getContextPath()%>/workflow/apply_leave_request/query.action">
<%
    }
    else{
%>
<form class="search_form" action="<%=request.getContextPath()%>/workflow/apply_leave_request/query.action?managerId=<%=((User)session.getAttribute("user")).getId()%>">
<%
    }
%>
...
(function($){
    ...
    initSearchView(id="workorder_apply_leave_request_view",table_colums,"id",dategrid_toolbar);
